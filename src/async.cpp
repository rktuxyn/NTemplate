...
NTemplate::Async::ParseTemplate(ThreadSContext *context) {
	...
	TemplateParser* tParser = new NTemplate::TemplateParser(context->rsinf, context->settings->pagePath.c_str(), context->settings->dir);
	malloc(sizeof(tParser));
	tParser->Start();
...
void NTemplate::Async::FinalizationTask(ThreadSContext * context, bool canRun) {
	...
	if (context->rsinf->is_error == true) {
		context->rsinf->t_source.clear();
		rsp->Set(v8::String::NewFromUtf8(isolate, "is_error"), v8::Boolean::New(isolate, context->rsinf->is_error));
		rsp->Set(v8::String::NewFromUtf8(isolate, "err_msg"), v8::String::NewFromUtf8(isolate, context->rsinf->err_msg.c_str()));
	}
	else {
		if (context->rsinf->is_script == true) {
			rsp->Set(v8::String::NewFromUtf8(isolate, "result"), NTemplate::Compiler::RunTemplateScript(isolate, context->rsinf->t_source, context->settings->data, canRun));
			context->rsinf->t_source.clear();
		}
		else {
			rsp->Set(v8::String::NewFromUtf8(isolate, "result"), v8::String::NewFromUtf8(isolate, context->rsinf->t_source.c_str()));
			context->rsinf->t_source.clear();
		}
		rsp->Set(v8::String::NewFromUtf8(isolate, "is_error"), v8::Boolean::New(isolate, false));
		rsp->Set(v8::String::NewFromUtf8(isolate, "err_msg"), v8::String::NewFromUtf8(isolate, ""));
...
NTemplate::Async::__DoAsyncTask(v8::Isolate* isolate, Settings* settings) {
	{
		...
		try {
			...
			context->rsinf = (TemplateResult*)malloc(sizeof(TemplateResult));
